...
class UserService:
    ...
    async def get_or_create_user(self, telegram_id: int, username: str = None,
        ...
        if not user:
            ...
            # ✅ ИСПРАВЛЕНИЕ: Принудительно генерируем user.id в базе данных
            await self.session.flush()

            # Теперь user.id гарантированно является числом, а не None!
...
class ErrorLogService:
    ...
    async def log_error(self, error_type: str, error_message: str, user_id: int = None):
        error_log = ErrorLog(
